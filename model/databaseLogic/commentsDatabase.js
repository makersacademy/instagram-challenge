...
class commentsDatabase {
  static async getComments(postId) {
    const comments = await connection.pool.query(
      "SELECT comments.id, comments.text, comments.created_at, comments.user_id, comments.post_id, users.username FROM comments INNER JOIN users ON comments.user_id=users.id WHERE post_id = $1 ORDER BY id DESC;",
      [postId]
    );
